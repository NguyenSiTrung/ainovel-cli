...
// stageSystemPrompt 组装阶段共创的完整系统提示：阶段 prompt + 对话语言指令 + 当前故事状态摘要。
// 对话语言指令追加在最后（离输出最近，优先级最高）；摘要作为数据附录挂在指令之前，
// 用分隔线与格式规范隔开，呼应 prompt 里"进度见下方"的指引。
func stageSystemPrompt(s *store.Store, lang string) string {
