...
func GhostCharacter(snap *Snapshot) []Finding {
	...
	for _, c := range snap.Characters {
		...
		gap := latest - seen
		if !ok {
			ghosts = append(ghosts, fmt.Sprintf(i18n.T("diag.rules.ghost_character.never_seen"), c.Name))
		} else if gap > threshold {
			ghosts = append(ghosts, fmt.Sprintf(i18n.T("diag.rules.ghost_character.absent_long"), c.Name, seen, gap))
		}
	}
	if len(ghosts) == 0 {
		return nil
	...
	return []Finding{{
		...
		Title:      fmt.Sprintf(i18n.T("diag.rules.ghost_character.title"), len(ghosts)),
		Evidence:   strings.Join(ghosts, "; "),
		Suggestion: i18n.T("diag.rules.ghost_character.sugg"),
	}}
}
...
func TimelineGaps(snap *Snapshot) []Finding {
	...
	if len(snap.Timeline) == 0 && snap.CompletedCount() > 0 {
		return []Finding{{
			...
			Title:      i18n.T("diag.rules.timeline_gaps.empty_title"),
			Evidence:   fmt.Sprintf("completed=%d, timeline_events=0", snap.CompletedCount()),
			Suggestion: i18n.T("diag.rules.timeline_gaps.empty_sugg"),
		}}
	}
	...
	return []Finding{{
		...
		AutoLevel:  AutoNone,
		Target:     "context.timeline",
		Title:      fmt.Sprintf(i18n.T("diag.rules.timeline_gaps.gap_title"), len(missing)),
		Evidence:   fmt.Sprintf("missing=[%s]", intsToStr(missing)),
		Suggestion: i18n.T("diag.rules.timeline_gaps.gap_sugg"),
	}}
}
...
func RelationshipStagnation(snap *Snapshot) []Finding {
	...
	return []Finding{{
		...
		Title:      fmt.Sprintf(i18n.T("diag.rules.relationship_stagnation.title"), latestRelCh),
		...
		Suggestion: i18n.T("diag.rules.relationship_stagnation.sugg"),
	}}
}
